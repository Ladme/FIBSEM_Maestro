...
def main():
    ...
    for i in range(3):
        imaging_settings = ImagingSettings.from_file(
            Path(args.imaging.replace(".yaml", f"{i}.yaml"))
        )
    ...
    imagings[0].collect_and_write_properties()

    microscope.beam.scanning_area = RelativeArea.full()
    microscope._control.try_set_stage_position(
        StagePosition(x=500, y=0, z=5_000_000.0, rotation=0, tilt=0)
    )

    imagings[1].collect_and_write_properties()
    ...
    imagings[2].collect_and_write_properties()
